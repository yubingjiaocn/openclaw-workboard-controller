...
- Persists only controller state under the OpenClaw plugin state dir: subscription id, processed event ids, terminal wake ids, legacy problem ids, notified start identities, bounded recent terminal wakes/failures, bounded start notifications/failures, counters, and last error. It does not persist Gateway credentials.
- Event order is explicit: read a contiguous batch, process and persist each terminal event, attempt the owner wake best-effort, dispatch ready cards once when the batch had new terminal events, then advance the Workboard subscription cursor with `workboard_notify_advance`.

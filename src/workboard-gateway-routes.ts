...
function createGatewayRouteHandler(method: string, normalizeBody: BodyNormalizer, dispatch: DispatchGatewayMethod) {
  return async (req: IncomingMessage, res: ServerResponse): Promise<boolean> => {
    ...
    try {
      params = normalizeBody(await readJsonBody(req));
    } catch (error) {
    ...
    try {
      const response = await dispatch(method, params, { expectFinal: true });
...
export function createWorkboardListRouteHandler(dispatch: DispatchGatewayMethod = dispatchGatewayMethod) {
  return createGatewayRouteHandler("workboard.cards.list", normalizeWorkboardListRequestBody, dispatch);
}
